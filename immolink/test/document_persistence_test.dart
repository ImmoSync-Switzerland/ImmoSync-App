...
void main() {
  group('Document Persistence Tests', () {
    ...
    test('Documents should persist after app restart simulation', () async {
      ...
      expect(documentsBeforeRestart.any((doc) => doc.id == 'test_doc_1'), isTrue);
      ...
      expect(documentsAfterRestart.any((doc) => doc.id == 'test_doc_1'), isTrue);
      
      final persistedDocument = documentsAfterRestart.firstWhere((doc) => doc.id == 'test_doc_1');
    ...
    test('Tenant should see documents assigned to their property', () async {
      ...
      final tenantDocuments = await documentService.getDocumentsForTenant('tenant_1');
      ...
      expect(tenantDocuments.any((doc) => doc.propertyIds.isEmpty && doc.assignedTenantIds.isEmpty), isTrue);
    });
    ...
    test('Document assignment should persist', () async {
      ...
      await documentService.assignDocumentToTenants('assign_test_doc', ['tenant_1', 'tenant_2']);
      await documentService.assignDocumentToProperties('assign_test_doc', ['property_1']);
      
      // Simulate app restart
      final newService = DocumentService();
      final tenantDocuments = await newService.getDocumentsForTenant('tenant_1');
      
      // Verify assignments persisted
      final assignedDocument = tenantDocuments.firstWhere((doc) => doc.id == 'assign_test_doc');
