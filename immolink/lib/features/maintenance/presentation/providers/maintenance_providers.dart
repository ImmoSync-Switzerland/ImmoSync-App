...
final propertyMaintenanceRequestsProvider = FutureProvider.family
    .autoDispose<List<MaintenanceRequest>, String>((ref, propertyId) async {
  final maintenanceService = ref.watch(maintenanceServiceProvider);
  return maintenanceService.getMaintenanceRequestsByProperty(propertyId);
});

// Provider for a single maintenance request by ID
final maintenanceRequestProvider = FutureProvider.family
    .autoDispose<MaintenanceRequest, String>((ref, requestId) async {
  final maintenanceService = ref.watch(maintenanceServiceProvider);
...
class MaintenanceRequestNotifier
    extends StateNotifier<AsyncValue<MaintenanceRequest?>> {
  final MaintenanceService _maintenanceService;

  MaintenanceRequestNotifier(this._maintenanceService)
  ...
  Future<void> createMaintenanceRequest(MaintenanceRequest request) async {
    ...
    try {
      final createdRequest =
          await _maintenanceService.createMaintenanceRequest(request);
  ...
  Future<void> updateMaintenanceRequest(MaintenanceRequest request) async {
    ...
    try {
      final updatedRequest =
          await _maintenanceService.updateMaintenanceRequest(request);
...
final maintenanceRequestNotifierProvider = StateNotifierProvider.autoDispose<
    MaintenanceRequestNotifier, AsyncValue<MaintenanceRequest?>>((ref) {
  final maintenanceService = ref.watch(maintenanceServiceProvider);
