...
class MatrixChatService {
  ...
  MobileMatrixClient? get _mobileClient =>
      _isRustBridgeSupported ? null : MobileMatrixClient.instance;
  ...
  Future<void> ensureInitialized({required String homeserver}) async {
    ...
    if (await markerFile.exists()) {
      print(
          '[MatrixChatService] Found delete marker, clearing old Matrix store...');
      try {
        ...
        print(
            '[MatrixChatService] Warning: Failed to delete old store: $deleteError');
    ...
    if (_isRustBridgeSupported) {
      ...
      try {
        ...
        if (msg.contains('already initialized') ||
            msg.contains('client already initialized')) {
      ...
      print(
          '[MatrixChatService] Using mobile Matrix client for ${defaultTargetPlatform.name}');
  ...
  Future<void> waitForFirstSyncEvent(
  ...
  Future<void> ensureReadyForUser(String userId) async {
    ...
    if (userId.isEmpty || userId == 'null' || userId == 'unknown-user') {
      print(
          '[MatrixChatService] ERROR: Invalid userId "$userId", skipping Matrix initialization');
    ...
    if (_initializationInProgress != null) {
      print(
          '[MatrixChatService] Initialization already in progress, waiting...');
    ...
    try {
      ...
      print(
          '[MatrixChatService] Fetching Matrix account from $api/matrix/account/$userId');
      ...
      Future<Map<String, dynamic>?> fetchAccount() async {
        ...
        if (r.statusCode == 503) {
          ...
          print(
              '[MatrixChatService] Matrix service not configured on backend (503)');
      ...
      if (data != null && data['configured'] == false) {
        print(
            '[MatrixChatService] WARNING: Matrix service not configured on backend');
        print(
            '[MatrixChatService] Chat will use HTTP-only mode (no real-time updates)');
        _readyUserId = userId;
      ...
      if (data == null) {
        ...
        print(
            '[MatrixChatService] Account not found, attempting to provision...');
        try {
          final headers = await _tokenManager.getHeaders();
          ...
          if (provisionResp.statusCode == 503 ||
              provisionResp.statusCode == 500) {
            print(
                '[MatrixChatService] WARNING: Matrix provisioning failed (server not configured)');
      ...
      if (data == null) {
        print(
            '[MatrixChatService] WARNING: Matrix account not available, using HTTP-only mode');
        _readyUserId = userId;
        _initializationInProgress = null;
        completer.complete();
        return; // Skip Matrix initialization
      ...
      final homeserver =
          (data['homeserver'] as String?) ?? (data['baseUrl'] as String?) ?? '';
      final username = data['username'] as String?;
      final password = data['password'] as String?;
      print(
          '[MatrixChatService] Credentials: homeserver=$homeserver, username=$username');
      print(
          '[MatrixChatService] Password length: ${password?.length ?? 0} chars');
      print(
          '[MatrixChatService] Password (first 8 chars): ${password?.substring(0, password.length > 8 ? 8 : password.length) ?? 'MISSING'}...');

      if (homeserver.isEmpty || username == null || password == null) {
        print(
            '[MatrixChatService] WARNING: Incomplete credentials, using HTTP-only mode');
        _readyUserId = userId;
        _initializationInProgress = null;
        completer.complete();
        return; // Skip Matrix initialization
      ...
      try {
        ...
        if (errorMsg.contains('account in the store doesn\'t match') ||
          ...
          print(
              '[MatrixChatService] Device mismatch during init, clearing store...');
          final dir = await getApplicationSupportDirectory();
          final storeDir = Directory(dir.path);
          if (await storeDir.exists()) {
      ...
      if (!_loggedIn) {
        ...
        try {
          ...
          if (errorMsg.contains('account in the store doesn\'t match') ||
            ...
            print(
                '[MatrixChatService] Device mismatch detected during login, clearing store and retrying...');
            try {
              ...
              for (int i = 0; i < retries && !deleted; i++) {
                if (i > 0) {
                  print(
                      '[MatrixChatService] Retry $i after ${i * 500}ms delay...');
                ...
                if (await storeDir.exists()) {
                  try {
                    print(
                        '[MatrixChatService] Deleting store directory: ${dir.path}');
                    await storeDir.delete(recursive: true);
                    deleted = true;
                    print(
                        '[MatrixChatService] Store deleted, reinitializing...');
                  } catch (deleteError) {
                    if (i == retries - 1) {
                      rethrow; // Rethrow on last retry
                    }
                    print(
                        '[MatrixChatService] Delete attempt ${i + 1} failed: $deleteError');
                  }
                } else {
              ...
              print(
                  '[MatrixChatService] Failed to recover from device mismatch: $clearError');
              final dir = await getApplicationSupportDirectory();

              // The store directory is locked by the Matrix client - force app restart
              print(
                  '[MatrixChatService] Store is locked, scheduling auto-restart...');
              ...
              try {
                ...
                print(
                    '[MatrixChatService] Failed to create marker: $markerError');
              }

              // Schedule app restart (platform-specific)
              print(
                  '[MatrixChatService] Requesting app restart to clear Matrix store...');
