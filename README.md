...
> See `backend/` and `immolink/` directories for server and Flutter sources respectively.
...
Android google services: create a Firebase project → download `google-services.json` (Android) → set one of the secrets above; CI writes it inside `immolink/android/app` before build.
...
App version is read from `immolink/pubspec.yaml` (`version:`). That raw string (e.g. `1.0.0+1`) is used for file naming. If introducing build metadata with characters that are problematic for shell/glob patterns, prefer restrict to `[0-9A-Za-z.+-]`.
