...
final landlordServicesProvider =
...
final allAvailableServicesProvider =
    FutureProvider.autoDispose<List<Service>>((ref) async {
  final serviceService = ref.watch(serviceServiceProvider);
...
final tenantAvailableServicesProvider = FutureProvider.autoDispose
    .family<List<Service>, String>((ref, landlordId) async {
  final serviceService = ref.watch(serviceServiceProvider);
...
final servicesByCategoryProvider = FutureProvider.autoDispose
    .family<List<Service>, String>((ref, category) async {
  final user = ref.watch(currentUserProvider);
  if (user == null) {
    throw Exception('User not authenticated');
  }
...
class ServiceNotifier extends StateNotifier<AsyncValue<Service?>> {
  ...
  ServiceNotifier(this._serviceService, this._ref)
...
final serviceNotifierProvider =
    StateNotifierProvider.autoDispose<ServiceNotifier, AsyncValue<Service?>>(
        (ref) {
  final serviceService = ref.watch(serviceServiceProvider);
